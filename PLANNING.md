...
| 2026-06-29 (cont) | **POS=eligible slots on two-start + ownership-velocity builders (cont7 follow-up #1 closed).** The last two flag builders that didn't thread eligibility — `compute_two_start_pitchers` and `compute_ownership_velocity` in `evaluate.py` — now `SELECT` and emit `eligible_slots` (matching the buy-low/sell-high pattern). Flows automatically into waivers.json (`two_starters_fa` filtered keeps dicts intact; `ownership_alerts` passes straight through `report.build_waivers`). `waivers.html`: two-start row already called `posSlots(p)` (was returning "—" for lack of data — now resolves); ownership-velocity row's "Pos" column was mislabeled (rendered `p.team`, an MLB team, under a "Pos" header) → switched to `posSlots(p)`. 2 code files. Verified vs live DB: ownership_alerts now carry eligible_slots (e.g. `3B,CI,UTIL,BN,IL,1B` → posSlots → `3B,CI,UTIL,1B`); two-starters empty this run (no probables posted yet, expected) but identical proven query path + compiles clean. Did NOT regenerate waivers.json locally — local db is gitignored/older than the June 29 cloud data, so regen would roll the live dashboard backward; next 6am cloud full run rebuilds it with the new code. | Visual POS render confirms after next 6am cloud run (waivers tab). Remaining cont7 fallback note fully closed. |
| 2026-06-29 | **Fix stale "last full/light run" on Log tab.** Cloud ran full daily (commits June 18–29 all [full], as_of advances) but Log showed last_full_run/last_light_run frozen at June 17. Cause: (1) `meta["last_full_run"]` set AFTER `report()` built pipeline-log.json → log captured prior run's value; (2) CI commits only `docs/data/`, not `data/pipeline-meta.json`, so each cloud run re-read the last committed meta (June 17 from manual local runs). Fix: move last_full_run/last_run_mode (and last_light_run) assignment + save BEFORE `report()` in both run_full/run_light, so the real date lands in the committed pipeline-log.json. Also updated CLAUDE.md hard-limits to the current once-daily 6am full schedule (was stale 3x-daily). Couldn't live-verify locally (sandbox network failed mid-transform); fix is a straightforward reorder and takes effect on the next cloud 6am run. last_light_run will now honestly sit at its last actual light run (none since schedule change). | Verify on Log tab after next 6am cloud run shows current date. |
| 2026-07-01 | **Fix: pipeline blocked since June 29 — crosswalk.integrity FAIL (4 rostered players without MLB ID > threshold 3).** Root cause: `validate.check_crosswalk_integrity` counts current-rostered players (LEFT JOIN dim_players) with NULL mlb_id; any FAIL calls `sys.exit(1)` ([validate.py:513](pipeline/validate.py:513)) → CI job non-zero → no `docs/data` commit (hence no data commits June 30 / July 1). The 4: **Jake McCarthy** (ESPN 41197, ARI OF — the "hit"/hitter) and **Gage Jump** (5023852, ATH LHP, debut 2026-05-26) were missing from the May crosswalk CSV entirely; **Luis Garcia Jr.** (40459, WSH 2B) and **Eduardo Rodriguez** (32675, LHP) were in the CSV with NULL mlb_id (ambiguous exact names — 5 Luis Garcías, 2 E-Rods). NOT caused by the June 29 code changes — pre-existing crosswalk drift from roster churn. Resolved all 4 MLB IDs authoritatively via MLB Stats API + pybaseball register (McCarthy 664983, Jump 695611, García Jr. 671277, Rodriguez 593958), disambiguating by team/position/active-years. **Fix (surgical, not a full rebuild):** filled the 2 null rows + appended the 2 missing rows in `data/player-crosswalk.csv` (status=manual), and added all 4 to `data/crosswalk-overrides.json` so the next weekly rebuild reproduces them. Did NOT weaken the check (CLAUDE.md forbids). Verified two ways: (a) live-roster coverage 0/255 uncovered vs edited CSV; (b) fresh CI-style seed (558 players) + real `check_crosswalk_integrity` on the June-29 snapshot that originally failed → now PASS. 2 tracked files. | Confirm the next 6am cloud run commits data again + Log tab advances. Longer-term: the committed CSV is a May snapshot — new rostered players need periodic `build_crosswalk` refresh (needs ESPN creds); consider automating a weekly refresh or raising CSV coverage. |
| 2026-07-29 | **Fix: "This Week's Matchup" showed UNKNOWN for every category since 2026-07-13 (ASG-break off-by-one).** Home/matchup tabs rendered all cats UNKNOWN with "— / —" (my_val/opp_val all None); opponent names + ERA/WHIP IP counters still worked. Root cause: `fetch_espn.fetch_matchups` filtered the current matchup with `matchupPeriodId != _current_week_number()`, and `_current_week_number()` is pure date math (`2 + days_since_w2 // 7`) with no All-Star-break handling. ESPN pauses matchupPeriodId over the break (`FIRST_HALF_END 07-12`, `ASG_BREAK_END 07-16`), so from 07-13 on the date week ran 1 ahead of ESPN's real period — the filter matched an empty *future* matchup (period 18, empty cumulativeScore) instead of the live one (period 17). Confirmed live: date-week=18, ESPN current period=17 (only 17 had cumulativeScore data). Fix: select the current period from ESPN's own data — the earliest UNDECIDED matchupPeriodId (completed weeks carry a winner) — instead of the date-based number, and set `result['current_week']` to it. Break-proof and self-correcting. Verified live: period 17 now returns real values (R 11-7, K 19-23, QS 1-3, …); end-to-end through `compute_cat_states` → 0 UNKNOWN (R/HR/SB FLOPPABLE, K/QS/SvHd FLIPPABLE). 1 file (fetch_espn.py). acq-counting was unaffected (it uses a date-based window aligned to the same dates, only the period *number* differed). | **Related, NOT yet fixed (flagged):** `report.py:509` recomputes `current_week` from the same date math (=18) for playoff timing → `weeks_until_playoffs` / `in_playoff_weeks` are off by one on the Playoff tab. Fix would source current_week from `matchup_snapshots` MAX(week) (now correct) — but build_playoff takes `league_data`, not a conn, so deferred pending go-ahead. |
| 2026-07-01 (cont) | **Built daily crosswalk self-heal (prevents the recurrence of today's failure).** New `pipeline/crosswalk_refresh.py` `heal()` runs in `main.py` right after `fetch_espn` (both full + light, non-fatal): (1) finds rostered players with no dim_players row or null mlb_id (same LEFT JOIN as validate); (2) if none → returns immediately (no ESPN re-fetch, no pybaseball download — the healthy case); (3) resolves each via crosswalk-overrides.json first (cheap), then only if needed downloads the pybaseball register and reuses `build_crosswalk.match_player` (exact → unique fuzzy); (4) upserts resolved rows into dim_players (fixes the current run) AND persists them to `data/player-crosswalk.csv` (line-oriented upsert: updates null rows in place / appends new, preserves line endings + other rows byte-for-byte, CSV-quotes notes with commas); (5) returns any still-ambiguous players as `unresolved` for a manual override — validate still gates those. Workflow: `daily-pipeline.yml` `git add` now also stages `data/player-crosswalk.csv` so the healed seed persists (and the register isn't re-downloaded for the same player next run). **Chose daily self-heal over a weekly rebuild** because today's break (McCarthy/Jump) was mid-week churn a Monday-only job would miss. Tested on temp DBs: override path (McCarthy/Jump/E-Rod), register path (Mike Trout → 545361), missing-from-dim insert, CSV update-in-place + append (comma-safe), and `check_crosswalk_integrity` → PASS after heal. 3 files: crosswalk_refresh.py (new), main.py, daily-pipeline.yml (hard-limit file — change was user-instructed). | Watch the next 6am run's log for a "crosswalk self-heal" line. Truly new *ambiguous* names (e.g. another "Luis Garcia") still need a manual crosswalk-overrides.json entry — heal flags them; only fails the run if >3 accumulate. Full weekly `build_crosswalk` rebuild remains available for a from-scratch refresh. |
| 2026-06-29 (backlog session) | **Cleared 4 backlog items (user approved >3-file scope upfront).** Each committed separately. (1) **Trade-scorer rate calibration** (`trade.py`, design §8 "the big one"): single-trade team-rate swings dwarfed counting cats (Muncy→Burns ERA+WHIP 0.30 vs 0.08 all counting; totals >1.0). Widened `RATE_MARGIN` ~2.5× (ERA .20→.50, WHIP .04→.10, OBP .010→.025). Re-validated: Muncy→Burns .372→.193, Duran→Davis Martin −.118→+.022; full run still 35 trades, top .655 (<1.0). Magnitudes now trustworthy; player_value rate term left alone (separately calibrated cont4, lineup sensible). (2) **Trades tab nav link** (7 HTML pages): trades.html shipped orphaned (only reachable by URL) — added `<a href="trades.html">Trades</a>` between Playoffs and Log on all 7 other pages; all 8 now cross-link. (3) **protected-players param** (`trade.py`): `PROTECTED_PLAYERS` set (empty default) threaded through `enumerate_trades(..., protected=)` — protected names never appear on the give side; verified 0 leakage. (4) **Statcast resilience** (`fetch_statcast.py`): FanGraphs is fetched before Savant, but a Savant failure returned early and discarded the fresh xFIP/SIERA/wRC+ (powers two-start + buy/sell). Now falls back to existing Savant cols from DB (`_existing_savant_frames`) and continues, so FanGraphs always refreshes + prior Savant preserved; `savant_skipped` recorded in cache/summary. Verified by forcing a Savant outage: FanGraphs refreshed, 519 Savant rows preserved, no crash. pybaseball Savant currently WORKS (583 rows live) — this is resilience hardening, not an outage fix. No API/field-mapping changes. Did NOT commit locally-regenerated trades.json (local db older than June 29 cloud data — would roll dashboard back); cloud regenerates with new calibration on next 6am run. | Verify after next 6am cloud run: Trades tab reachable from nav; trades.json magnitudes <1.0; waivers POS renders. **Open (deferred, design §8):** asset/upside valuation (model under-prices ace pedigree — needs ESPN-projection/scarcity blend, its own design decision); value-window blend; VORP sanity baseline. **Side-finding (not fixed):** statcast-cache.json lives in `data/` which CI doesn't commit, so the 7-day TTL is effectively dead in the cloud (re-fetches Savant every run) — same class as the June 29 pipeline-meta issue; would need a workflow change to fix. |
